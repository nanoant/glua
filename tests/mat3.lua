...
print(string.format('testing mat3 %d interations:', iter)); io.stdout:flush()

-- matrix * constant
local a = gl.identity3
...
-- matrix * vector
local a = gl.rotate3(math.pi/2,-math.pi/2,math.pi/2)
local v = gl.vec3(1,2,3)
local start = os.clock()
for i = 1, iter do
  v = a * v
end
...
-- matrix * matrix
local a = gl.identity3
local b = gl.rotate3(math.pi/2,-math.pi/2,math.pi/2)
local start = os.clock()
for i = 1, iter do
  a = a * b
end
